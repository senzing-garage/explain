...
1. Run program.
   ...
   1. macOS
      ...
      ```console
      ${GIT_REPOSITORY_DIR}/target/darwin-amd64/explain
   ...
   1. Windows
      ...
      ```console
      ${GIT_REPOSITORY_DIR}/target/windows-amd64/explain
...
1. Senzing documentation will be in the "Third party" section.
   `github.com` > `senzing-garage` > `explain`
...
1. Determine if `explain` is installed.
   Example:

   ```console
   apt list --installed | grep explain

   ```

1. :pencil2: Install `explain`.
   Example:
...
1. Remove `explain` from system.
   Example:

   ```console
   sudo apt-get remove explain
